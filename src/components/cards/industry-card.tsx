...
export function IndustryCard({ slug, title, description, icon, compact }: IndustryCardProps) {
  ...
  if (compact) {
    return (
      <Link
        to={`/industries/${slug}`}
        className="group block bg-white rounded-xl border border-gray-200 p-4 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-center"
      >
        {/* Icon */}
  ...
  return (
    <Link
      to={`/industries/${slug}`}
      className="group block bg-white rounded-xl border border-gray-200 p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
    >
      {/* Icon */}
